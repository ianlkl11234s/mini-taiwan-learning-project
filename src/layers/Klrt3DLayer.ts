...
import type { KlrtTrain } from '../engines/KlrtTrainEngine';
import type { Track } from '../types/track';
import { getKlrtLineId, getKlrtDirection, KLRT_LINE_COLORS_3D } from '../constants/klrtInfo';

// 參考點：高雄市中心
const MODEL_ORIGIN: [number, number] = [120.31, 22.63];

// 高雄輕軌列車尺寸（公尺）- 輕軌列車較小
const TRAIN_LENGTH = 70;   // 長度 (前後方向)
const TRAIN_WIDTH = 28;    // 寬度 (左右方向)
const TRAIN_HEIGHT = 28;   // 高度 (垂直方向)

/**
 * Klrt3DLayer - Mapbox Custom Layer for 3D KLRT train rendering
 */
export class Klrt3DLayer implements mapboxgl.CustomLayerInterface {
  id = 'klrt-3d-layer';
  type: 'custom' = 'custom';
  renderingMode: '3d' = '3d';
  ...
  updateTrains(trains: KlrtTrain[]): void {
    this.trains = trains;
  }
  ...
  onAdd(map: mapboxgl.Map, gl: WebGLRenderingContext): void {
    ...
    // 建立環狀線的材質
    // C 線 Direction 0 (順時針 - 標準綠)
    const materialC0 = new THREE.MeshStandardMaterial({
      color: KLRT_LINE_COLORS_3D.C,
      transparent: true,
      opacity: 0.9,
      emissive: KLRT_LINE_COLORS_3D.C,
      emissiveIntensity: 0,
    });
    // C 線 Direction 1 (逆時針 - 淺綠)
    ...
    this.materials.set('C_0', materialC0);
    this.materials.set('C_1', materialC1);
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      let closestTrain: KlrtTrain | null = null;
  ...
  private updateTrainMeshes(): void {
    ...
    for (const train of this.trains) {
      ...
      const material = this.materials.get(materialKey) || this.materials.get('C_0')!;
  ...
  private calculateBearing(train: KlrtTrain): number {
