...
// 台鐵列車尺寸（公尺）- 介於高鐵與捷運之間
const TRAIN_LENGTH = 200;  // 長度 (前後方向)
const TRAIN_WIDTH = 85;    // 寬度 (左右方向)
const TRAIN_HEIGHT = 80;   // 高度 (垂直方向)

/**
 * Tra3DLayer - Mapbox Custom Layer for 3D TRA train rendering
 *
 * 使用 O-D 專屬軌道系統，支援內灣線 (NW) 和六家線 (LJ)
 */
export class Tra3DLayer implements mapboxgl.CustomLayerInterface {
  id = 'tra-3d-layer';
  type: 'custom' = 'custom';
  renderingMode: '3d' = '3d';
  ...
  private updateTrainMeshes(): void {
    ...
    for (const train of this.trains) {
      ...
      let bearing: number;
      if (train.status === 'stopped' && train.currentStation) {
        bearing = this.calculateStationBearing(train.currentStation, train.odTrackId);
      } else {
  ...
  private calculateStationBearing(stationId: string, odTrackId: string): number {
    const stationCoord = this.stationCoordinates.get(stationId);
    if (!stationCoord) return 0;

    // 使用對應的 O-D 軌道
    const track = this.odTracks.get(odTrackId);
    if (!track) return 0;
