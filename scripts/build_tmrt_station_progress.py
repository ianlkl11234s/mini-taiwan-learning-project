...
使用方式：
    python scripts/build_tmrt_station_progress.py

輸入：
    - data-tmrt/raw/tmrt_station_of_line.json

輸出：
    - public/data-tmrt/station_progress.json
"""
...
# 路徑設定
RAW_DIR = Path(__file__).parent.parent / "data-tmrt" / "raw"
OUTPUT_DIR = Path(__file__).parent.parent / "public" / "data-tmrt"
OUTPUT_FILE = OUTPUT_DIR / "station_progress.json"

# 軌道配置
TRACKS = [
    ('TMRT-G-0', 'G', 0),  # 綠線 Direction 0 (北屯→高鐵)
    ('TMRT-G-1', 'G', 1),  # 綠線 Direction 1 (高鐵→北屯)
]


def load_station_of_line() -> List[Dict[str, Any]]:
    """載入車站線序資料"""
    with open(RAW_DIR / "tmrt_station_of_line.json", 'r', encoding='utf-8') as f:
        return json.load(f)
