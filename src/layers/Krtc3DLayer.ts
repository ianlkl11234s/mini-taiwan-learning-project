...
import { getKrtcLineId, getKrtcDirection, KRTC_LINE_COLORS_3D } from '../constants/krtcInfo';

// 參考點：高雄市中心
const MODEL_ORIGIN: [number, number] = [120.31, 22.63];

// 高雄捷運列車尺寸（公尺）
const TRAIN_LENGTH = 120;  // 長度 (前後方向)
const TRAIN_WIDTH = 40;    // 寬度 (左右方向)
const TRAIN_HEIGHT = 35;   // 高度 (垂直方向)

/**
 * Krtc3DLayer - Mapbox Custom Layer for 3D KRTC train rendering
 */
export class Krtc3DLayer implements mapboxgl.CustomLayerInterface {
  id = 'krtc-3d-layer';
  type: 'custom' = 'custom';
  renderingMode: '3d' = '3d';
  ...
  updateTrains(trains: KrtcTrain[]): void {
    this.trains = trains;
  }
  ...
  onAdd(map: mapboxgl.Map, gl: WebGLRenderingContext): void {
    ...
    // 建立各線路的材質
    // 橘線 Direction 0 (深橘)
    const materialO0 = new THREE.MeshStandardMaterial({
      color: KRTC_LINE_COLORS_3D.O,
      transparent: true,
      opacity: 0.9,
      emissive: KRTC_LINE_COLORS_3D.O,
      emissiveIntensity: 0,
    });
    // 橘線 Direction 1 (淺橘)
    const materialO1 = new THREE.MeshStandardMaterial({
      color: 0xffc266,
      transparent: true,
      opacity: 0.9,
      emissive: 0xffc266,
    ...
    // 紅線 Direction 0 (深紅)
    const materialR0 = new THREE.MeshStandardMaterial({
      color: KRTC_LINE_COLORS_3D.R,
      transparent: true,
      opacity: 0.9,
      emissive: KRTC_LINE_COLORS_3D.R,
      emissiveIntensity: 0,
    });
    // 紅線 Direction 1 (淺紅)
    const materialR1 = new THREE.MeshStandardMaterial({
      color: 0xff7a7a,
      transparent: true,
      opacity: 0.9,
      emissive: 0xff7a7a,
    ...
    this.materials.set('O_0', materialO0);
    this.materials.set('O_1', materialO1);
    this.materials.set('R_0', materialR0);
    this.materials.set('R_1', materialR1);
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      let closestTrain: KrtcTrain | null = null;
  ...
  private updateTrainMeshes(): void {
    ...
    for (const train of this.trains) {
      ...
      const materialKey = `${lineId}_${direction}`;
      const material = this.materials.get(materialKey) || this.materials.get('O_0')!;
  ...
  private calculateBearing(train: KrtcTrain): number {
