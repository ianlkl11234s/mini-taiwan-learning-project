...
從 TDX Shape 資料 (WKT LINESTRING) 生成校準後的軌道 GeoJSON：
- TMRT-G-0.geojson (綠線 北屯總站→高鐵台中站)
- TMRT-G-1.geojson (綠線 高鐵台中站→北屯總站)

Usage:
    python scripts/build_tmrt_tracks.py
"""
...
OUTPUT_DIR = Path(__file__).parent.parent / "public" / "data-tmrt" / "tracks"
STATIONS_FILE = Path(__file__).parent.parent / "public" / "data-tmrt" / "stations" / "tmrt_stations.geojson"
...
LINES_CONFIG = {
    'G': {
        'name': '綠線',
        'color': '#0cab2c',
        'direction_0': '北屯總站 → 高鐵台中站',
        'direction_1': '高鐵台中站 → 北屯總站',
        'start_station': 'G0',
        'end_station': 'G17',
    }
}
...
def parse_wkt_to_segments(wkt: str) -> List[List[List[float]]]:
    """
    解析 WKT LINESTRING 或 MULTILINESTRING 為線段列表
    """
    wkt = wkt.strip()
    segments = []
...
def connect_segments(segments: List[List[List[float]]], tolerance: float = 0.001) -> List[List[float]]:
    ...
    while remaining:
        ...
        if best_idx == -1:
            print(f"  warning: 無法找到匹配的線段，剩餘 {len(remaining)} 個線段未連接")
            break
...
def parse_wkt_linestring(wkt: str) -> List[List[float]]:
    """
    解析 WKT LINESTRING/MULTILINESTRING 為座標陣列
    """
...
def load_shape_by_line(line_id: str) -> str:
    """載入指定線路的軌道幾何"""
    with open(RAW_DIR / "tmrt_shape.json", 'r', encoding='utf-8') as f:
        data = json.load(f)
...
def main():
    ...
    for line_id, config in LINES_CONFIG.items():
        ...
        if not stations:
            print(f"  warning: 找不到 {line_id} 線車站")
            continue

        wkt = load_shape_by_line(line_id)
        if not wkt:
            print(f"  warning: 找不到 {line_id} 線軌道")
            continue

        original_coords = parse_wkt_linestring(wkt)
        print(f"  原始座標點數: {len(original_coords)}")

        # 建立 Direction 0 (正向: 北屯→高鐵)
        track_id_0 = f"TMRT-{line_id}-0"
        ...
        print(f"    saved: {output_path_0.name}")

        # 建立 Direction 1 (反向: 高鐵→北屯)
        track_id_1 = f"TMRT-{line_id}-1"
        ...
        print(f"    saved: {output_path_1.name}")
    ...
    print("=" * 50)
    print(f"\n產出檔案位於: {OUTPUT_DIR}")
    print("\n下一步：執行 build_tmrt_schedules.py 建立時刻表")
