...
Usage:
    python scripts/fetch_klrt_data.py
"""
...
# 輸出目錄
OUTPUT_DIR = Path(__file__).parent.parent / "data-klrt" / "raw"
...
def fetch_klrt_data():
    ...
    with open(OUTPUT_DIR / "klrt_stations.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "klrt_shape.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "klrt_station_of_line.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "klrt_station_timetable.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "klrt_s2s_travel_time.json", 'w', encoding='utf-8') as f:
        json.dump(s2s_travel_time, f, ensure_ascii=False, indent=2)
    print(f"      儲存 {len(s2s_travel_time)} 筆站間時間")

    print("\n=== 下載完成 ===")
    print(f"資料儲存於: {OUTPUT_DIR}")
    print(f"  - klrt_stations.json")
    print(f"  - klrt_shape.json")
    print(f"  - klrt_station_of_line.json")
    print(f"  - klrt_station_timetable.json")
    print(f"  - klrt_s2s_travel_time.json")
    ...
    print("\n環狀線特性:")
    if station_of_line:
        line = station_of_line[0]
        stations_list = line.get('Stations', [])
        if stations_list:
            first_station = stations_list[0]
            last_station = stations_list[-1]
            print(f"  起點站: {first_station.get('StationID')} ({first_station.get('StationName', {}).get('Zh_tw', '')})")
            print(f"  終點站: {last_station.get('StationID')} ({last_station.get('StationName', {}).get('Zh_tw', '')})")
            total_distance = last_station.get('CumulativeDistance', 0)
            print(f"  總長度: {total_distance:.2f} km")
