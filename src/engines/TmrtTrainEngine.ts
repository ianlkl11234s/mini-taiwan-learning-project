/**
 * TmrtTrainEngine - 台中捷運列車引擎
 *
 * 簡化版列車引擎：
 * - 單一路線（綠線）
 * - 無碰撞檢測（TMRT 軌道獨立）
 * - 無延長日制（TMRT 06:00-24:00 運營）
 * - 使用累積距離計算車站進度
 */

import type { TrackSchedule, StationTime } from '../types/schedule';
import type { Track } from '../types/track';
import type { TmrtStationProgressMap } from '../hooks/useTmrtData';

export interface TmrtTrain {
  trainId: string;
  trackId: string;
...
export interface TmrtTrainEngineOptions {
...
export class TmrtTrainEngine {
  private schedules: Map<string, TrackSchedule>;
  private tracks: Map<string, Track>;
  private activeTrains: Map<string, TmrtTrain> = new Map();
  private stationProgress: TmrtStationProgressMap = {};

  constructor(options: TmrtTrainEngineOptions) {
   ...
   */
  setStationProgress(progress: TmrtStationProgressMap): void {
   ...
   */
  update(currentTimeSeconds: number): TmrtTrain[] {
  ...
  getStats(): {
    ...
    let running = 0;
    let stopped = 0;
    const byLine: Record<string, number> = { G: 0 };
