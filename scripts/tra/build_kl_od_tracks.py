#!/usr/bin/env python3
"""
build_kl_od_tracks.py - 建立 KL (基隆支線) O-D 專屬軌道

基隆支線路線：
- 臺北 (1000) → ... → 八堵 (0920) → 三坑 (0910) → 基隆 (0900)
- 與宜蘭線在八堵分岔，運營完全獨立
"""
...
OD_ROUTES = [
    # 臺北 → 基隆
    ODRoute(
        od_track_id='KL-TP-KL',
        origin_station_id='1000',
        destination_station_id='0900',
        origin_name='臺北',
        destination_name='基隆',
        segments=[
            ('WL-N-0', '1000', '0900'),  # 臺北→基隆 (西部幹線含基隆支線)
        ]
    ),
    # 基隆 → 臺北
    ODRoute(
        od_track_id='KL-KL-TP',
        origin_station_id='0900',
        destination_station_id='1000',
        origin_name='基隆',
        destination_name='臺北',
        segments=[
            ('WL-N-1', '0900', '1000'),  # 基隆→臺北
        ]
    ),
]
...
def load_stations() -> Dict[str, Station]:
    ...
    # 確認基隆相關車站存在
    kl_stations = ['1000', '0990', '0980', '0970', '0960', '0950', '0940', '0930', '0920', '0910', '0900']
    for sid in kl_stations:
        if sid in stations:
            print(f"  ✓ {sid} {stations[sid].name}: {stations[sid].coordinates}")
        else:
            print(f"  ✗ {sid} 車站不存在！")
...
def get_all_stations_for_route(route: ODRoute) -> List[str]:
    """取得路由經過的所有車站 ID"""
    # 基隆線只有一段，直接返回 KL_STATIONS
    if route.od_track_id == 'KL-TP-KL':
        return KL_STATIONS.copy()
    elif route.od_track_id == 'KL-KL-TP':
        return KL_STATIONS[::-1]  # 反向
    return []
...
def main():
    ...
    print("=" * 60)
    print("\n下一步:")
    print("1. 執行 fetch_yl_kl_timetable.py 取得 TDX 時刻表")
    print("2. 更新 useTraData.ts 和 TraTrainEngine.ts")
