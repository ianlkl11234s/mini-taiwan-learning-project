...
Usage:
    python scripts/fetch_krtc_data.py
"""
...
# 輸出目錄
OUTPUT_DIR = Path(__file__).parent.parent / "data-krtc" / "raw"
...
def fetch_krtc_data():
    ...
    with open(OUTPUT_DIR / "krtc_stations.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "krtc_shape.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "krtc_station_of_line.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "krtc_station_timetable.json", 'w', encoding='utf-8') as f:
    ...
    with open(OUTPUT_DIR / "krtc_s2s_travel_time.json", 'w', encoding='utf-8') as f:
        json.dump(s2s_travel_time, f, ensure_ascii=False, indent=2)
    print(f"      儲存 {len(s2s_travel_time)} 筆站間時間")

    print("\n=== 下載完成 ===")
    print(f"資料儲存於: {OUTPUT_DIR}")
    print(f"  - krtc_stations.json")
    print(f"  - krtc_shape.json")
    print(f"  - krtc_station_of_line.json")
    print(f"  - krtc_station_timetable.json")
    print(f"  - krtc_s2s_travel_time.json")
