...
Problem: 112 tracks / 333 trains (35%) have a loop near Changhua station,
...
def main():
    tracks_dir = os.path.join(BASE, "tracks_od")
    ...
    ms_path = os.path.join(BASE, "schedules_real", "master_schedule.json")
    track_counts = {}
    if os.path.exists(ms_path):
        with open(ms_path) as f:
            ms = json.load(f)
        for train in ms["schedules"]:
            tid = train.get("od_track_id", "")
    ...
    # Recalculate od_station_progress
    print("\n=== Recalculating od_station_progress ===")
    ...
    if os.path.exists(sp_script):
        ...
        print("  fix-station-progress.py not found, skipping")
