...
OD_ROUTES = [
    ...
    # SH 線 O-D (沙崙線)
    # SH-0 方向是中洲→沙崙 (手繪軌道)
    # SH-1 方向是沙崙→中洲 (手繪軌道反向)
    # WL-SH-0 方向是大湖→仁德 (手繪軌道)
    # WL-SH-1 方向是仁德→大湖 (手繪軌道反向)
    ...
    ODRoute(
        ...
        destination_name='沙崙',
        segments=[
            ('WL-S1-1', '4220', '4260'),  # 臺南→仁德 (縱貫線南下)
            ('WL-SH-1', '4260', '4270'),  # 仁德→中洲 (手繪西部幹線)
            ('SH-0', '4270', '4272'),     # 中洲→沙崙 (手繪沙崙線)
        ]
    ),
    ODRoute(
        ...
        destination_name='臺南',
        segments=[
            ('SH-1', '4272', '4270'),     # 沙崙→中洲 (手繪沙崙線反向)
            ('WL-SH-0', '4270', '4260'),  # 中洲→仁德 (手繪西部幹線)
            ('WL-S1-0', '4260', '4220'),  # 仁德→臺南 (縱貫線北上)
        ]
    ),
]
...
def load_stations() -> Dict[str, Station]:
    ...
    # 修正集集線車站座標 - 從完整軌道（含手繪補充）計算的 snapped 座標
    # 軌道已從二水延伸到車埕，共 626 個座標點
    jj_station_corrections = {
        '3430': (120.621691, 23.811299),   # 二水 - 軌道起點
        '3431': (120.642109, 23.798522),   # 源泉
        '3432': (120.704797, 23.834514),   # 濁水
        '3433': (120.750037, 23.835246),   # 龍泉
        '3434': (120.784961, 23.826318),   # 集集
        '3435': (120.853343, 23.818671),   # 水里
        '3436': (120.867393, 23.833916),   # 車埕 - 軌道終點
    }
