/**
 * KlrtTrainEngine - 高雄輕軌列車引擎
 *
 * 輕軌列車引擎：
 * - 單一環狀線（C 線）
 * - 順時針 (Direction 0) / 逆時針 (Direction 1)
 * - 起終點相同（C1 籬仔內）
 * - 無碰撞檢測（輕軌軌道獨立）
 */
...
import type { KlrtStationProgressMap } from '../hooks/useKlrtData';

export interface KlrtTrain {
  trainId: string;
  trackId: string;
...
export interface KlrtTrainEngineOptions {
...
export class KlrtTrainEngine {
  private schedules: Map<string, TrackSchedule>;
  private tracks: Map<string, Track>;
  private activeTrains: Map<string, KlrtTrain> = new Map();
  private stationProgress: KlrtStationProgressMap = {};

  constructor(options: KlrtTrainEngineOptions) {
   ...
   */
  setStationProgress(progress: KlrtStationProgressMap): void {
   ...
   */
  update(currentTimeSeconds: number): KlrtTrain[] {
    ...
    for (const [trackId, schedule] of this.schedules) {
      ...
      for (const departure of schedule.departures) {
        ...
        // 環狀線特殊處理：終點站（最後一個 C1）的進度應該是 1.0
        const isFromLastStation = segment.stationIndex === totalStations - 1;
        const isToLastStation = segment.nextStationIndex === totalStations - 1;

        const fromProgress = isFromLastStation
          ? 1.0  // 終點站進度為 1.0
          : (fromStationId && trackProgress?.[fromStationId] !== undefined
            ? trackProgress[fromStationId]
            : this.getStationProgressFallback(segment.stationIndex, totalStations));
        const toProgress = isToLastStation
          ? 1.0  // 終點站進度為 1.0
          : (toStationId && trackProgress?.[toStationId] !== undefined
        ...
        if (displayStatus === 'stopped') {
          // 停站中：使用該站的進度在軌道上定位
          // 注意：終點站需要使用 1.0 進度，確保列車停在軌道末端
          const stoppedProgress = isFromLastStation ? 1.0 : fromProgress;
          position = interpolateOnLineString(coords, stoppedProgress);
        } else {
          // 行駛中：在兩站進度間內插，沿軌道移動
  ...
  getStats(): {
    ...
    let running = 0;
    let stopped = 0;
    const byLine: Record<string, number> = { C: 0 };
