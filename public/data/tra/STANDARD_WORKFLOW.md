...
> 建立時間：2026-01-13
> 最後更新：2026-01-25 (Step 22 深澳線完成，TRA 軌道建置完成)
...
### 8. 車站投影標準化（⭐ 重要）
- ✅ **停車位置必須在軌道上**：站點座標必須位於軌道線段上
- ✅ **站點標示與停車位置相同**：圓點標記位置 = 列車停站位置
- ⚠️ 若站點偏離軌道，需將站點**投影**到軌道上
- ⚠️ 投影後的座標需**插入**軌道座標陣列中
...
# 對所有站點執行投影，更新：
# 1. stations_snapped.geojson（站點座標）
# 2. 軌道 GeoJSON（插入投影點）
# 3. od_station_progress.json（重新計算 progress）
```

**驗收標準**：
- 站點到軌道距離 < 1m（約 0.00001 度）
...
| 站點標記偏離停車位置 | 站點未投影到軌道上 | 執行車站投影標準化流程（參考第 8 點） |
...
Step 15: 屏東線 PT-ZY-PL (新左營→枋寮，經高雄) ✅
    │
    ↓
Step 18: 西部幹線海線 WL-C (竹南→彰化) ✅ 完成
    │
    ↓
Step 19: 西部幹線南段 WL-S (彰化→新左營) ✅
    │
    ↓
Step 20: 西部幹線山線 WL-M (竹南→彰化) ✅
    │
    ↓
Step 21: 西部幹線北段 WL-N (竹南→樹林) ✅ 完成
    │
    ↓
Step 22: 深澳線 SA (瑞芳→八斗子) ✅ 完成
    │
    ↓
Step 23: 全線合併與最終驗證 ✅ (TRA 軌道建置完成)
```

### 建置完成總結

**已完成路線**：22 條 (含支線)
- 東部幹線：YL 宜蘭線、BH 北迴線、TL 臺東線
- 西部幹線：WL-N 北段、WL-M 山線、WL-C 海線、WL-S 南段
- 南迴線：SK 南迴線
- 屏東線：PT 屏東線
- 支線：NW 內灣線、LJ 六家線、SH 沙崙線、JJ 集集線、CZ 成追線、PX 平溪線、SA 深澳線、KL 基隆支線

**後續工作**：
1. 將測試時刻表替換為 TDX 真實時刻表
2. 優化列車渲染效能
3. 新增更多列車類型（自強號、普悠瑪、太魯閣等）
