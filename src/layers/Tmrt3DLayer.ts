...
import { getTmrtLineId, getTmrtDirection, TMRT_LINE_COLORS_3D } from '../constants/tmrtInfo';

// 參考點：台中市中心
const MODEL_ORIGIN: [number, number] = [120.68, 24.15];

// 台中捷運列車尺寸（公尺）
const TRAIN_LENGTH = 80;   // 長度 (前後方向)
const TRAIN_WIDTH = 30;    // 寬度 (左右方向)
const TRAIN_HEIGHT = 30;   // 高度 (垂直方向)

/**
 * Tmrt3DLayer - Mapbox Custom Layer for 3D TMRT train rendering
 */
export class Tmrt3DLayer implements mapboxgl.CustomLayerInterface {
  id = 'tmrt-3d-layer';
  type: 'custom' = 'custom';
  renderingMode: '3d' = '3d';
  ...
  updateTrains(trains: TmrtTrain[]): void {
    this.trains = trains;
  }
  ...
  onAdd(map: mapboxgl.Map, gl: WebGLRenderingContext): void {
    ...
    // 建立綠線的材質
    // G 線 Direction 0 (往高鐵台中站 - 標準綠)
    const materialG0 = new THREE.MeshStandardMaterial({
      color: TMRT_LINE_COLORS_3D.G,
      transparent: true,
      opacity: 0.9,
      emissive: TMRT_LINE_COLORS_3D.G,
      emissiveIntensity: 0,
    });
    // G 線 Direction 1 (往北屯總站 - 淺綠)
    const materialG1 = new THREE.MeshStandardMaterial({
      color: 0x4ddb75,
      transparent: true,
      opacity: 0.9,
      emissive: 0x4ddb75,
    ...
    this.materials.set('G_0', materialG0);
    this.materials.set('G_1', materialG1);
    ...
    const handleClick = (event: MouseEvent) => {
      ...
      let closestTrain: TmrtTrain | null = null;
  ...
  private updateTrainMeshes(): void {
    ...
    for (const train of this.trains) {
      ...
      const material = this.materials.get(materialKey) || this.materials.get('G_0')!;
  ...
  private calculateBearing(train: TmrtTrain): number {
