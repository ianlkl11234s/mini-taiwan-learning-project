/**
 * ThsrTrainEngine - 台灣高鐵列車引擎
 *
 * 簡化版列車引擎：
 * - 單一路線（南港-左營）
 * - 無碰撞檢測（高鐵軌道獨立）
 * - 無延長日制（高鐵 06:00-23:00 運營）
 * - 使用累積距離計算車站進度
 */
...
export interface ThsrTrainEngineOptions {
...
export class ThsrTrainEngine {
   ...
   * 使用均勻分布（高鐵軌道已簡化）
   */
  private getStationProgress(stationIndex: number, totalStations: number): number {
   ...
   */
  update(currentTimeSeconds: number): ThsrTrain[] {
